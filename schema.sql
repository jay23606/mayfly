...
-- Friend-only durable activity. Live presence remains the source of truth for
-- "online"; this timestamp is the fallback after presence disappears.
...
create policy "mf_user_activity_select" on public.mf_user_activity for select to authenticated using (
  auth.uid() = user_id or exists (select 1 from public.mf_friends f where f.status = 'accepted'
...
create policy "mf_user_activity_insert" on public.mf_user_activity for insert to authenticated with check (auth.uid() = user_id);
drop policy if exists "mf_user_activity_update" on public.mf_user_activity;
create policy "mf_user_activity_update" on public.mf_user_activity for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
create or replace function public.mf_touch_activity() returns void language sql security invoker set search_path = public as $$
  insert into public.mf_user_activity (user_id, last_active_at) values (auth.uid(), now())
...
create or replace function public.mf_friend_activity() returns table(user_id uuid, last_active_at timestamptz)
language sql stable security definer set search_path = public as $$
  select a.user_id, a.last_active_at from public.mf_user_activity a where a.user_id = auth.uid() or exists (
    select 1 from public.mf_friends f where f.status = 'accepted'
      and ((f.requester_id = auth.uid() and f.addressee_id = a.user_id) or (f.addressee_id = auth.uid() and f.requester_id = a.user_id))
  );
$$;
...
create policy "mf_devices_select" on public.mf_devices for select to authenticated using (
  revoked_at is null and (auth.uid() = user_id or exists (select 1 from public.mf_friends f where f.status = 'accepted'
    and ((f.requester_id = auth.uid() and f.addressee_id = user_id) or (f.addressee_id = auth.uid() and f.requester_id = user_id))))
);
...
-- Origin-scoped Web Push endpoints can move safely between accounts using the
-- same browser; only the currently authenticated owner can release one.
...
begin
  ...
  values(auth.uid(),push_endpoint,push_p256dh,push_auth,now()) on conflict(endpoint) do update
...
create or replace function public.mf_release_push_subscription(push_endpoint text) returns void
language sql security definer set search_path=public as $$ delete from public.mf_push_subscriptions where user_id=auth.uid() and endpoint=push_endpoint; $$;
