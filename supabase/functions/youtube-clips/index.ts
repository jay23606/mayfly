...
serve(async (request) => {
  ...
  try {
    const { query = "funny", pageToken = "", channelId = "" } = await request.json();
    const key = Deno.env.get("YOUTUBE_API_KEY");
    if (!key) throw new Error("YouTube search is not configured");
    const search = new URL("https://www.googleapis.com/youtube/v3/search");
    search.search = new URLSearchParams({ key, part: "snippet", q: String(query).slice(0, 100), type: "video", maxResults: "12", videoEmbeddable: "true", safeSearch: "moderate", ...(channelId ? { channelId: String(channelId).slice(0, 80) } : {}), ...(pageToken ? { pageToken: String(pageToken) } : {}) }).toString();
    ...
    const items = (searchJson.items || []).filter((item: any) => allowed.has(item.id?.videoId)).map((item: any) => ({ provider: "youtube", videoId: item.id.videoId, name: decodeTitle(item.snippet.title), channel: decodeTitle(item.snippet.channelTitle), channelId: item.snippet.channelId || "", thumbnail: item.snippet.thumbnails?.high?.url || item.snippet.thumbnails?.medium?.url || "" }));
